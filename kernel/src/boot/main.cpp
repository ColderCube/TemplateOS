...
__attribute__((used, section(".requests"))) volatile uint64_t base_revision[3] =
...
__attribute__((used, section(".requests"))) volatile limine_module_request
...
__attribute__((used, section(".requests_start_marker"))) volatile uint64_t
    requests_start_marker[4] = LIMINE_REQUESTS_START_MARKER;

__attribute__((
    used,
    section(".requests_end_marker"))) volatile uint64_t requests_end_marker[2] =
...
extern "C" void kmain() {
  ...
  *>(framebuffer->address); fb_ptr[i * (framebuffer->pitch / 4) + i] = 0xffffff;
  }*/
